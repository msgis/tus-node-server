...
class PostHandler extends BaseHandler {
    ...
    send(req, res) {
        return this.store.create(req)
            .then((File) => {
                let url = this.store.relativeLocation ? `${req.baseUrl || ''}${this.store.path}/${File.id}` : `//${req.headers.host}${req.baseUrl || ''}${this.store.path}/${File.id}`;
                if (this.store.baseUrl) {
                    url = `${this.store.baseUrl}${this.store.path}/${File.id}`;
                }
